  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const ev of rows as any[]) {
        ev.registrations = (ev.registrations || []).sort((a: any, b: any) => (b.created_at || "").localeCompare(a.created_at || ""));
      }
      return rows;
    },
  });
        ...
        <section key={ev.id} className="rounded-lg border border-border bg-card overflow-hidden">
          ...
          {!ev.registrations?.length ? <p className="p-6 text-center text-sm text-muted-foreground">No registrations yet.</p> : (
            <table className="w-full text-sm">
              <thead className="bg-secondary text-left"><tr><th className="p-3">Name</th><th className="p-3">Email</th><th className="p-3">Reg. Number</th><th className="p-3">Faculty</th><th className="p-3">Year</th><th className="p-3">Ticket</th><th className="p-3">Registered</th></tr></thead>
              <tbody>{ev.registrations.map((r: any) => (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="p-3">{r.year_of_study || "—"}</td>
                  <td className="p-3"><span className="rounded bg-accent px-2 py-0.5 text-xs">{r.ticket_tier}</span></td>
                  <td className="p-3 text-xs text-muted-foreground">{formatShortDate(r.created_at)}</td>
                </tr>
              ))}</tbody>
            </table>
          )}
